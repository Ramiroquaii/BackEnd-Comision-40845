...
function readHtmlLFile(htmlFile) {        
  const fileData =  fs.readFileSync(`./pages/${htmlFile}.html`, 'utf-8');
  return fileData;
}
...
io.on('connection', client => {
  ...
  client.on('login-user', logUser => {
    // verificacion de usuario en BD y GENERACION de SESION.
    if(logUser.user == "Ramiro"){
      var htmlCodeUpdate = readHtmlLFile('mainPage');
      client.emit('reload', htmlCodeUpdate);
    } else {

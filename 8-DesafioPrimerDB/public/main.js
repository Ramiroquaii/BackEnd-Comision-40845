...
socket.on('messages', data => {
  ...
  data.forEach(message => {
    html += `<div class="chatLine" li><p class="user">${message.user}&nbsp;</p> <p class="time">[ ${message.timestamp} ] :&nbsp;</p> <p class="msg">${message.mensaje}</p></div>`
  });
...
socket.on('products', data => {
  ...
  data.forEach(element => {
    ...
    //<p class="stockTd">$ ${element.stock}</p>
  });
